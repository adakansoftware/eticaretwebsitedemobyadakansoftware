...
export default async function OrderSuccessPage({ params, searchParams }: OrderSuccessPageProps) {
  ...
  const detailHref = order.userId
    ? `/orders/${order.id}`
    : `/orders/${order.id}?access=${encodeURIComponent(access ?? "")}`;
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-12">
        <div className="rounded-3xl bg-white p-8 shadow-sm">
          ...
          {!order.userId ? (
            <p className="mt-2 text-slate-700">
              Misafir siparisi icin bu sayfayi kaydedebilirsin; link siparis ozetini tekrar acar.
            </p>
          ) : null}
